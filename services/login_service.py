...
class LoginService:
    def login(self): # Realiza o login devolvendo um token
                ...
                if member: # Checka se o user existe
                    if member.active: # Checka se o user esta ativo
                        if check_password_hash(pwd, member.hash): # Checka o HASH da Senha 
                            token = create_token({ "member_id": str(member.id), "partnership_id": str(member.partnership_id), "client_id": str(member.client_id) }) # Cria o access token
                            return {
                                "access_token": token,
        ...
        return jsonify("Email obrigatorio"), 400 # Email obrigatoria 400, BAD REQUEST
